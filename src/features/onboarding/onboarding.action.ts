...
export const saveActivityProfileAction = authAction
  .inputSchema(
    z.object({
      sportHoursPerWeek: z.number().min(0).max(40).optional(),
...
const FamilyMemberInputSchema = z.object({
  ...
  birthYear: z.number().min(1900).max(new Date().getFullYear()).optional(),
  weightKg: z.number().min(5).max(300).optional(),
  heightCm: z.number().min(50).max(250).optional(),
  sportTypes: z.array(z.string()).default([]),
  activityLevel: z
    .enum(["sedentary", "light", "moderate", "active", "very_active"])
...
export const saveFamilyMembersAction = authAction
  .inputSchema(
    z.object({
      members: z.array(FamilyMemberInputSchema),
    }),
  )
  .action(async ({ parsedInput: { members }, ctx: { user } }) => {
    // Remplace tous les membres existants par les nouveaux
...
// ─── Étape 4 : Préférences alimentaires (mise à jour par membre) ──────────────
...
export const saveMemberDietaryPrefsAction = authAction
  ...
  .action(async ({ parsedInput: { memberId, dietaryPrefs }, ctx: { user } }) => {
    // Vérifie que le membre appartient bien à cet utilisateur
